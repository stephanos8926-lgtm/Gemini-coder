...
renderer.code = ({ text, lang }: { text: string; lang?: string }) => {
  ...
  return `
    <div class="code-block-wrapper relative group my-4 rounded-lg overflow-hidden border border-border-subtle bg-[#0d0d0d]" data-large="${isLarge}">
      <div class="flex items-center justify-between px-3 py-1.5 bg-[#1a1a1a] border-b border-border-subtle">
        <span class="text-[10px] font-bold text-text-subtle uppercase tracking-widest">${language}</span>
        <button aria-label="Copy code" class="copy-btn p-1 hover:bg-surface-accent rounded transition-colors focus-visible:ring-1 focus-visible:ring-accent-intel outline-none" data-code="${encodeURIComponent(text)}">
      ...
      ${isLarge ? `
        <button aria-label="Expand code" class="expand-btn absolute bottom-0 left-0 right-0 py-2 bg-gradient-to-t from-[#0d0d0d] to-transparent text-[10px] font-bold text-accent-intel hover:opacity-80 transition-all flex items-center justify-center gap-1 focus-visible:ring-1 focus-visible:ring-accent-intel outline-none">
          <span>Show More</span>
        </button>
      ` : ''}
    </div>
              ...
              {RW_contextSnippets.map((s, i) => (
                <div key={i} className="flex items-center gap-1.5 px-2 py-1 bg-accent-intel/10 border border-accent-intel/30 rounded-md">
                  ...
                  <button
                    ...
                    className="text-text-subtle hover:text-accent-destructive transition-colors focus-visible:ring-1 focus-visible:ring-accent-intel outline-none rounded"
