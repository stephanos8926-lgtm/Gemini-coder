...
renderer.code = ({ text, lang }: { text: string; lang?: string }) => {
  ...
  return `
    <div class="code-block-wrapper relative group my-4 rounded-lg overflow-hidden border border-border-subtle bg-surface-base" data-large="${isLarge}">
      <div class="flex items-center justify-between px-3 py-1.5 bg-surface-card border-b border-border-subtle">
        <span class="text-[10px] font-bold text-text-subtle uppercase tracking-widest">${language}</span>
        <button class="copy-btn p-1 hover:bg-surface-accent rounded transition-colors focus-visible:ring-1 focus-visible:ring-accent-intel outline-none" data-code="${encodeURIComponent(text)}" aria-label="Copy code">
      ...
      ${isLarge ? `
        <button class="expand-btn absolute bottom-0 left-0 right-0 py-2 bg-gradient-to-t from-surface-base to-transparent text-[10px] font-bold text-accent-intel hover:opacity-80 transition-all flex items-center justify-center gap-1 focus-visible:ring-1 focus-visible:ring-accent-intel outline-none">
          <span>Show More</span>
        </button>
      ` : ''}
    </div>
    ...
    <div className="flex flex-col h-full w-full overflow-hidden bg-surface-base border-r border-border-subtle">
      ...
      <div className="px-4 py-3 border-b border-border-subtle bg-surface-card flex items-center justify-between shadow-sm z-10 shrink-0">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`flex items-center gap-1.5 px-2 py-1 rounded-md border transition-all focus-visible:ring-2 focus-visible:ring-accent-intel outline-none ${
      ...
      <div className="p-4 bg-transparent shrink-0">
        <div className="max-w-3xl mx-auto mb-2 flex items-center justify-between">
          <div className="flex items-center gap-2">
            {agents.length > 0 && (
              <div className="flex items-center gap-1.5 px-2 py-1 bg-surface-card border border-border-subtle rounded-md transition-all focus-within:ring-2 focus-within:ring-accent-intel/50">
        ...
        <form onSubmit={handleSubmit} className="relative group max-w-3xl mx-auto">
          ...
          <div className="relative flex items-end bg-surface-card border border-border-subtle rounded-2xl focus-within:border-accent-primary focus-within:ring-1 focus-within:ring-accent-primary/20 transition-all shadow-lg">
            <textarea
              ...
              className="w-full bg-transparent px-4 py-3 pr-12 text-sm text-text-primary placeholder-text-subtle focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-intel outline-none resize-none min-h-[44px] max-h-[200px] custom-scrollbar rounded-2xl"
              rows={1}
            ...
            <button
              ...
              className={`absolute right-2 bottom-2 p-2 rounded-xl transition-all focus-visible:ring-2 focus-visible:ring-accent-intel outline-none ${
                input.trim() && !isStreaming 
                  ? 'bg-accent-intel text-white hover:opacity-90'
