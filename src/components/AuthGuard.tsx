...
const SignInView = ({ onSignInGithub, onSignInGoogle }: any) => (
  <div className="absolute inset-0 z-[100] flex items-center justify-center bg-[#1e1e1e]/90 backdrop-blur-md">
    ...
    >
      <div className="w-16 h-16 bg-accent-intel/10 rounded-full flex items-center justify-center mx-auto mb-6">
        <LogIn className="w-8 h-8 text-accent-intel" />
      </div>
...
const LoadingView = () => (
  <div className="absolute inset-0 z-[100] flex items-center justify-center bg-[#1e1e1e]/90 backdrop-blur-md">
    <div className="flex flex-col items-center gap-4">
      <Loader2 className="w-12 h-12 text-accent-intel animate-spin" />
...
const SelectWorkspaceView = ({ onShowWorkspaceModal }: any) => (
  <div className="absolute inset-0 z-[100] flex items-center justify-center bg-[#1e1e1e]/90 backdrop-blur-md">
    ...
    >
      <div className="w-16 h-16 bg-accent-intel/10 rounded-full flex items-center justify-center mx-auto mb-6">
        <FolderOpen className="w-8 h-8 text-accent-intel" />
      </div>
      ...
      <button 
        onClick={onShowWorkspaceModal}
        className="w-full py-3 bg-accent-intel hover:bg-accent-intel/90 text-white rounded-lg font-semibold transition-all flex items-center justify-center gap-2 shadow-lg shadow-accent-intel/20"
      >
