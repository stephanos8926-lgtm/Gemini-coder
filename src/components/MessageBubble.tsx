      ...
      {functionCalls.map((call, idx) => (
        <div key={`call-${idx}-${call.name}`} className="bg-surface-card border border-border-subtle rounded-md overflow-hidden transition-all">
          <button
            ...
            aria-label={expanded === idx ? "Collapse tool details" : "Expand tool details"}
            className="w-full flex items-center justify-between px-2.5 py-1.5 hover:bg-surface-accent focus-visible:ring-2 focus-visible:ring-accent-intel focus:outline-none transition-colors"
          >
            ...
            {expanded === idx && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                <div className="p-2.5 border-t border-border-subtle bg-surface-base">
                  <pre className="text-[10px] text-accent-ops font-mono overflow-x-auto whitespace-pre-wrap break-all">{JSON.stringify(call.args, null, 2)}</pre>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
  ...
  <div className="w-full space-y-2 my-2">
    ...
    <div className="border-l-2 border-accent-intel/30 pl-3 py-1">
      <button
        ...
        className="flex items-center gap-2 mb-1 opacity-60 hover:opacity-100 focus-visible:ring-2 focus-visible:ring-accent-intel focus:outline-none rounded-md px-1 transition-opacity"
      >
      ...
      return (
        <div key={`resp-${idx}-${res.name}`} className={`bg-surface-base border ${isError ? 'border-accent-security/20' : 'border-border-subtle'} rounded-lg overflow-hidden shadow-sm`}>
          <div className="flex items-center gap-2 px-3 py-1.5 bg-surface-card border-b border-border-subtle">
            {isError ? <AlertCircle className="w-3 h-3 text-accent-security" /> : <CheckCircle2 className="w-3 h-3 text-green-400" />}
      ...
      if (filename.includes('.') && !['diff', 'delete', 'rename'].includes(filename)) {
        return (
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 bg-accent-intel/10 border border-accent-intel/30 text-accent-intel rounded-md hover:bg-accent-intel/20 focus-visible:ring-2 focus-visible:ring-accent-intel focus:outline-none transition-all text-[10px] font-bold uppercase tracking-wider"
          >
  ...
  <div className="mt-5 p-4 bg-surface-base border border-border-subtle rounded-xl shadow-inner">
    ...
    <div className="flex flex-wrap gap-2">
      <button
        ...
        className="flex-1 sm:flex-none px-4 py-2 bg-accent-intel text-white rounded-lg hover:bg-accent-intel/90 focus-visible:ring-2 focus-visible:ring-accent-intel focus:outline-none transition-all text-xs font-bold shadow-lg shadow-accent-intel/20 disabled:opacity-50"
      >
        Yes, proceed
      </button>
      <button
        ...
        className="flex-1 sm:flex-none px-4 py-2 bg-transparent text-text-primary border border-border-subtle rounded-lg hover:bg-surface-accent focus-visible:ring-2 focus-visible:ring-accent-intel focus:outline-none transition-all text-xs font-bold disabled:opacity-50"
      >
        No, cancel
      </button>
    </div>
