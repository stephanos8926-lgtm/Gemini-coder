    ...
    <div className="w-full space-y-1.5 mt-2">
      {functionCalls.map((call, idx) => (
        <div key={`call-${idx}-${call.name}`} className="bg-[#252526] border border-[#3c3c3c] rounded-md overflow-hidden transition-all">
          <button onClick={() => setExpanded(expanded === idx ? null : idx)} className="w-full flex items-center justify-between px-2.5 py-1.5 hover:bg-[#2d2d2d] transition-colors focus-visible:bg-[#2d2d2d] focus:outline-none">
...
const ThinkingBlock = ({ thinking, taskList, expanded, setExpanded }: any) => (
  <div className="w-full space-y-2 my-2">
    ...
    <div className="border-l-2 border-accent-intel/30 pl-3 py-1">
      <button onClick={setExpanded} className="flex items-center gap-2 mb-1 opacity-60 hover:opacity-100 transition-opacity focus:outline-none focus-visible:opacity-100">
      ...
      if (filename.includes('.') && !['diff', 'delete', 'rename'].includes(filename)) {
        return (
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 bg-accent-intel/10 border border-accent-intel/30 text-accent-intel rounded-md hover:bg-accent-intel/20 transition-all text-[10px] font-bold uppercase tracking-wider focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-intel"
          >
  ...
  <div className="mt-5 p-4 bg-[#1e1e1e] border border-[#3c3c3c] rounded-xl shadow-inner">
    ...
    <div className="flex flex-wrap gap-2">
      <button
        ...
        className="flex-1 sm:flex-none px-4 py-2 bg-accent-intel text-white rounded-lg hover:opacity-90 transition-all text-xs font-bold shadow-lg shadow-accent-intel/20 disabled:opacity-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-intel"
      >
        Yes, proceed
      </button>
      <button
    ...
    >
      <div className={`w-8 h-8 rounded-lg shrink-0 flex items-center justify-center border shadow-sm ${
        isUser 
          ...
          : 'bg-[#252526] border-[#3c3c3c] text-accent-intel'
      }`}>
        {isUser ? (
          settings.userAvatar ? (
