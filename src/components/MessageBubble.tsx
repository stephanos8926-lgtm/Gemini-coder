    ...
    <div className="w-full space-y-1.5 mt-2">
      {functionCalls.map((call, idx) => (
        <div key={`call-${idx}-${call.name}`} className="bg-surface-card border border-border-subtle rounded-md overflow-hidden transition-all">
          <button onClick={() => setExpanded(expanded === idx ? null : idx)} className="w-full flex items-center justify-between px-2.5 py-1.5 hover:bg-surface-accent transition-colors focus-visible:ring-2 focus-visible:ring-accent-intel outline-none" aria-label="Toggle tool call details">
            ...
            {expanded === idx && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                <div className="p-2.5 border-t border-border-subtle bg-surface-base">
                  <pre className="text-[10px] text-accent-intel font-mono overflow-x-auto whitespace-pre-wrap break-all">{JSON.stringify(call.args, null, 2)}</pre>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
...
const ThinkingBlock = ({ thinking, taskList, expanded, setExpanded }: any) => (
  <div className="w-full space-y-2 my-2">
          ...
          {taskList.split('\n').map((task: string, idx: number) => {
            ...
            return (
              <div key={`task-${idx}`} className="flex items-start gap-2 text-[11px]">
                <span className={`font-mono shrink-0 ${isDone ? 'text-green-500' : isInProg ? 'text-accent-intel animate-pulse' : isBlocked ? 'text-accent-security' : 'text-text-subtle'}`}>
                  {task.match(/\[.\]/)?.[0] || '[ ]'}
                </span>
    ...
    <div className="border-l-2 border-accent-intel/30 pl-3 py-1">
      <button onClick={setExpanded} className="flex items-center gap-2 mb-1 opacity-60 hover:opacity-100 transition-opacity focus-visible:ring-2 focus-visible:ring-accent-intel outline-none rounded" aria-label="Toggle thinking process">
    ...
    {responses.map((res: any, idx: number) => {
      ...
      return (
        <div key={`resp-${idx}-${res.name}`} className={`bg-surface-base border ${isError ? 'border-accent-security/20' : 'border-border-subtle'} rounded-lg overflow-hidden shadow-sm`}>
          ...
          <div className={`text-[11px] font-mono overflow-x-auto whitespace-pre-wrap break-all p-3 bg-black/10 ${isError ? 'text-accent-security/80' : 'text-text-primary'}`}>
      ...
      if (filename.includes('.') && !['diff', 'delete', 'rename'].includes(filename)) {
        return (
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 bg-accent-intel/10 border border-accent-intel/30 text-accent-intel rounded-md hover:bg-accent-intel/20 transition-all text-[10px] font-bold uppercase tracking-wider focus-visible:ring-2 focus-visible:ring-accent-intel outline-none"
  ...
  <div className="mt-5 p-4 bg-surface-base border border-border-subtle rounded-xl shadow-inner">
    ...
    <div className="flex flex-wrap gap-2">
      <button
        ...
        className="flex-1 sm:flex-none px-4 py-2 bg-accent-intel text-white rounded-lg hover:opacity-90 transition-all text-xs font-bold shadow-lg shadow-accent-intel/20 disabled:opacity-50 focus-visible:ring-2 focus-visible:ring-accent-intel outline-none"
      >
        Yes, proceed
      </button>
      <button
        ...
        className="flex-1 sm:flex-none px-4 py-2 bg-transparent text-text-primary border border-border-subtle rounded-lg hover:bg-surface-accent transition-all text-xs font-bold disabled:opacity-50 focus-visible:ring-2 focus-visible:ring-accent-intel outline-none"
      >
        No, cancel
      </button>
    </div>
      ...
      <div className={`flex flex-col max-w-[95%] sm:max-w-[85%] min-w-0 space-y-1 ${isUser ? 'items-end' : 'items-start'}`}>
        ...
        <div className={`px-4 py-3 rounded-2xl text-sm leading-relaxed relative group break-words max-w-full overflow-hidden shadow-sm ${
          isUser 
            ...
            : 'bg-surface-card text-text-primary border-border-subtle rounded-tl-none'
        }`}>
