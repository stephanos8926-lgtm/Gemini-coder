      ...
      {functionCalls.map((call, idx) => (
        <div key={`call-${idx}-${call.name}`} className="bg-surface-card border border-border-subtle rounded-md overflow-hidden transition-all">
          <button
            ...
            className="w-full flex items-center justify-between px-2.5 py-1.5 hover:bg-surface-accent transition-colors focus-visible:ring-2 focus-visible:ring-accent-intel outline-none"
...
const ThinkingBlock = ({ thinking, taskList, expanded, setExpanded }: any) => (
  <div className="w-full space-y-2 my-2">
    ...
    <div className="border-l-2 border-accent-intel/30 pl-3 py-1">
      <button
        onClick={setExpanded}
        className="flex items-center gap-2 mb-1 opacity-60 hover:opacity-100 transition-opacity focus-visible:ring-2 focus-visible:ring-accent-intel outline-none rounded-sm"
        aria-label={expanded ? 'Hide AI thought process' : 'View AI thought process'}
      >
  ...
  <div className="mt-5 p-4 bg-surface-base border border-border-subtle rounded-xl shadow-inner">
    ...
    <div className="flex flex-wrap gap-2">
      <button
        ...
        className="flex-1 sm:flex-none px-4 py-2 bg-accent-intel text-white rounded-lg hover:bg-accent-intel/90 transition-all text-xs font-bold shadow-lg shadow-accent-intel/20 disabled:opacity-50 focus-visible:ring-2 focus-visible:ring-accent-intel outline-none"
      ...
      <button
        ...
        aria-label="Cancel and do not proceed"
      >
        No, cancel
      </button>
    </div>
